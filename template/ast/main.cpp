...
//
// Here's a binder that produces a unary predicate; is this type similar to
// Shape?
//
...
//
// Here's a class template that has a constraint on the type used for
// specialization.
...
class InterfaceOnly2 {
    static_assert(satisfies<T>(isInterfaceClass),
                  "template argument must be an interface class.");
    static_assert(satisfies<T>(IsInterfaceClass()),
                  "template argument must be an interface class.");
    // implementation...
};
...
template <typename T1, typename T2,
          typename = Constraint<IsInterfaceClass, T1, T2>>
...
class InterfacesOnly2 {
    static_assert(satisfies<T1, T2, T3>(isInterfaceClass),
                  "template arguments must be interface classes.");
    // implementation...
};
...
class InterfacesOnly3 {
    static_assert(satisfies<Ts...>(isInterfaceClass),
                  "template arguments must be interface classes.");
    // implementation...
};
...
//
constexpr auto isShapeInterfaceClass = isInterfaceClass &
                                       bind2nd<std::is_base_of, Shape>() &
                                       !bind2nd<std::is_same, Shape>();
...
public:
    ...
    void doit(T const &a) {
        std::cout << "What::doit(lvalue): " << typeid(T).name() << std::endl;
    }
    void doit(T &&a) {
        std::cout << "What::doit(rvalue): " << typeid(T).name() << std::endl;
    }
    ...
    //
    // Class interface use #2: distinguish on secondary properties of args in an
    // overload.
    //    Distinguish different stl iterator categories from each other and from
    //    raw pointers.
    ...
    template <typename Rand,
              typename = Constraint<decltype(isRand & !isPointer), Rand>,
              typename = Ignore<1>>
    What(Rand b, Rand e) {
        std::cout << "What::What(Rand, Rand)" << std::endl;
    }

    template <typename Ptr, typename = Constraint<IsPointer, Ptr>,
              typename = Ignore<1>, typename = Ignore<2>>
    What(Ptr b, Ptr e) {
        std::cout << "What::What(Ptr, Ptr)" << std::endl;
    }

    //
    // Class interface use #3: distinguish on secondary properties of args in an
    // overload.
    // Here we're using a constraint with an AST object (for convenience, so we
    // don't need
    // to use decltype).
    //
    ...
    template <typename Rand, bool = constraint<Rand>(isRand & !isPointer),
              typename = Ignore<1>>
    ...
    static constexpr auto cast_constraint = isInterfaceClass &
                                            bind1st<std::is_base_of, Shape>() &
                                            !bind2nd<std::is_same, Shape>();
    ...
    void justdoit(Ts &&... args) {
        std::cout << "Universal #1 signed: " << sizeof...(args)
                  << " Constraint (should be true): "
                  << satisfies<Ts...>(isSigned) << std::endl;
    }

    template <typename... Ts, typename = Constraint<IsUnsigned, Ts...>,
              typename = void>
    void justdoit(Ts &&... args) {
        std::cout << "Universal #2 unsigned: " << sizeof...(args)
                  << " Constraint (should be true): "
                  << satisfies<Ts...>(isUnsigned) << std::endl;
    }

    template <typename... Ts, typename = Constraint<IsClass, Ts...>,
              typename = void, typename = void>
    void justdoit(Ts &&... args) {
        std::cout << "Universal #3 class: " << sizeof...(args)
                  << " Constraint (should be true): "
                  << satisfies<Ts...>(isClass) << std::endl;
    }
    ...
    template <typename... Ts, bool = constraint<Ts...>(isUnsigned),
              typename = void>
    void justdidit(Ts &&... args) {
        std::cout << "Universal #2 unsigned: " << sizeof...(args)
                  << " Constraint (should be true): "
...
void misc_tests() {
    ...
    cout << "Shape is interface (true): " << boolalpha << shape_is_interface
         << endl;

    constexpr bool circle_is_interface = satisfies<Circle>(isInterfaceClass);
    cout << "Circle is interface (false): " << boolalpha << circle_is_interface
         << endl;

    constexpr bool square_is_interface = isInterfaceClass.eval<Square>();
    cout << "Square is interface (true): " << boolalpha
         << isInterfaceClass.eval<Square>() << endl;

    constexpr bool triangle_is_interface =
        satisfies<Triangle>(isInterfaceClass);
    cout << "Triangle is interface (false): " << boolalpha
         << satisfies<Triangle>(isInterfaceClass) << endl;

    constexpr bool shape_is_big_interface =
        satisfies<Shape>(isInterfaceClass & !isSmall);
    cout << "Shape is big interface (false): " << boolalpha
         << shape_is_big_interface << endl;

    constexpr bool square_is_big_interface =
        satisfies<Square>(isInterfaceClass & !isSmall);
    cout << "Square is big interface (false): " << boolalpha
         << square_is_big_interface << endl;
    ...
    cout << "unsigned long long is too_complex (true): " << boolalpha << r6
         << endl;
}
}
...
void test_binders() {
    ...
    constexpr auto shapeish =
        bind1st<is_base_of, Shape>() | bind2nd<is_base_of, Shape>();
    constexpr auto r3 = satisfies<T>(shapeish);
    cout << typeid(T).name() << " is a Shape or a Shape is it: " << boolalpha
         << r3 << endl;
}
...
void test_expressions() {
    ...
    cout << typeid(T).name() << " is standard layout but not pod: " << boolalpha
         << r1 << endl;

    constexpr auto isInterfaceClass =
        isPtrSize & hasVirtualDestructor & isAbstract;
    constexpr auto r2 = satisfies<T>(isInterfaceClass);
    cout << typeid(T).name() << " is interface class: " << boolalpha << r2
         << endl;

    constexpr auto isConcreteShape = bind1st<is_base_of, Shape>() & !isAbstract;
    constexpr auto r3 = satisfies<T>(isConcreteShape);
    cout << typeid(T).name() << " is concrete Shape: " << boolalpha << r3
         << endl;
}
