...
struct all_derived_from<Base, F, Rs...> {
    enum : bool {
        value = std::is_base_of<Base, F>::value &&
                all_derived_from<Base, Rs...>::value
    };
};
...
//====================
// AST leaf nodes:
//     a) ids that wrap a predicate template that can be specilaized with one
//     typename
...
//
// Note use of variadic template template to allow predicates that take trailing
// defaults.
//
...
// used as an Id.
//
// Bind the first argument of a binary predicate that takes [typename,
// typename].
//
...
// Bind the second argument of a of a binary predicate that takes [typename,
// typename].
//
