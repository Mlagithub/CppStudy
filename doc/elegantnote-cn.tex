%!TEX program = xelatex
\documentclass[cn,geye,blue,14pt,normal]{elegantnote}
...
\begin{itemize}
  ...
  \item 
    \begin{itemize} 
      \item \cpp{ Widget w[maxNumWidgets];} $\rightarrow$ 发生构造及拷贝
      \item \cpp{ vector<Widget> vw;         } $\rightarrow$ 只是定义，按需要拷贝
      \item \cpp{ vector<Widget> vw; vw.reserve(maxNumWidgets); } $\rightarrow$ 预留空间，但不发生拷贝
    \end{itemize}
\end{itemize}
